    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                data = json.load(f)
                # ensure all sources are lowercase
                return [x.lower() for x in data.get("source_channels", [])], data.get("destination_channel")
...
def save_config(source_channels, destination_channel):
    try:
        with open(CONFIG_FILE, "w") as f:
            json.dump({
                "source_channels": [x.lower() for x in source_channels],  # store as lowercase
...
# --- Username-only, case-insensitive source channel matching ---
def chat_filter(chat):
    uname = getattr(chat, "username", None)
    return uname and uname.lower() in source_channels
